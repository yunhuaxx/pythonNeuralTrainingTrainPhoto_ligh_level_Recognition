...
// ImageClassificationDlg.cpp: 实现文件
//

#include "pch.h"
#include "framework.h"
#include "ImageClassification.h"
#include "ImageClassificationDlg.h"
...
BEGIN_MESSAGE_MAP(CImageClassifyDlg, CDialogEx)
	...
	ON_EN_CHANGE(IDC_EDIT_DARK_NUMS, &CImageClassifyDlg::OnEnChangeEditDarkNums)
	ON_EN_CHANGE(IDC_EDIT_BACKLINGHTING_NUMS, &CImageClassifyDlg::OnEnChangeEditBacklinghtingNums)
	ON_EN_CHANGE(IDC_EDIT_NORMAL_NUMS, &CImageClassifyDlg::OnEnChangeEditNormalNums)
	ON_EN_CHANGE(IDC_EDIT_DIMLIGHT_DIR, &CImageClassifyDlg::OnEnChangeEditDimlightDir)
	ON_BN_CLICKED(IDC_tc, &CImageClassifyDlg::OnBnClickedtc)
	ON_WM_CTLCOLOR()
	ON_WM_NCHITTEST()
	ON_EN_CHANGE(IDC_EDIT_SINGLE, &CImageClassifyDlg::OnEnChangeEditSingle)
	ON_WM_CTLCOLOR()//按键透明化
END_MESSAGE_MAP()
...
BOOL CImageClassifyDlg::OnInitDialog()
{
	...
	// TODO: 在此添加额外的初始化代码
	//CString sPath = _T("C:\\Users\\XYN\\Pictures\\bh3rd\\2022-08-28-19-11-47_0.png");//背景图位置ImageClassify
	CString sPath = _T("ryzr.png");
		CImage img;
	  img.Load(sPath);
	...
	//
		 // 设置按钮透明背景
	   // CButton* pButton = (CButton*)GetDlgItem(IDC_BUTTON_SINGLE);
		//pButton->ModifyStyle(0, BS_OWNERDRAW);
		//CWnd* pStatic = GetDlgItem(IDC_BUTTON_SINGLE); // 替换为文本控件的ID
		//pStatic->ModifyStyle(0, SS_CENTER); // 将文本控件的样式修改为居中显示
...
HBRUSH CImageClassifyDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	/*if (nCtlColor == CTLCOLOR_BTN && pWnd->GetDlgCtrlID() == IDC_BUTTON_SINGLE)
	{
	...
	*/
	...
	if (pWnd->GetDlgCtrlID() == IDC_STATIC ||
		pWnd->GetDlgCtrlID() == IDC_STATIC_SINGLE ||
		pWnd->GetDlgCtrlID() == IDC_STATIC_DIR) 
	{
	...
	/*
	if (pWnd->GetDlgCtrlID() == (IDC_STATIC_SINGLE))
	{
		//MessageBox(_T("static text"));
		pDC->SetBkMode(TRANSPARENT);
		pDC->SetTextColor(RGB(0, 0, 0));
...
LRESULT CImageClassifyDlg::OnNcHitTest(CPoint point)
{
...
void CImageClassifyDlg::OnEnChangeEditDarkNums()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 CDialogEx::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
...
void CImageClassifyDlg::OnEnChangeEditBacklinghtingNums()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 CDialogEx::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
...
void CImageClassifyDlg::OnEnChangeEditNormalNums()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 CDialogEx::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
...
void CImageClassifyDlg::OnEnChangeEditDimlightDir()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 CDialogEx::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
...
void CImageClassifyDlg::OnBnClickedtc()
{
	// TODO: 在此添加控件通知处理程序代码
	CDialogEx::OnCancel();
}
...
void CImageClassifyDlg::OnEnChangeEditSingle()
{
	// TODO:  如果该控件是 RICHEDIT 控件，它将不
	// 发送此通知，除非重写 CDialogEx::OnInitDialog()
	// 函数并调用 CRichEditCtrl().SetEventMask()，
